...
INTEGRITY_SCHEMA_VERSION = "replaybench-pg-integrity-v2"
RECEIPT_SCHEMA_VERSION = "replaybench-pg-receipt-v2"
...
def build_run_identity(
    *,
    ...
    manifest = {
        ...
        "checkpoint_sha256": (_safe_file_hash(checkpoint_path) if str(policy_mode) == "bc_live" else ""),
        "bc_action_sha256": (_safe_file_hash(bc_action_path) if str(policy_mode) == "bc" else ""),
        "generation_mode": str(generation_cfg.get("mode", "affective_response")),
        "downstream_operation": str(generation_cfg.get("service_name", "generation_stub")),
        "task_retry_enabled": bool(execution_cfg.get("task_retry_enabled", False)),
        "task_failure_policy": str(execution_cfg.get("task_failure_policy", "fail_fast")),
    }
    manifest_hash = sha256_json(manifest)
    trace_scope_manifest = {key: value for key, value in manifest.items() if key != "workers"}
    trace_scope_hash = sha256_json(trace_scope_manifest)
    configuration_id = f"rbpg-{manifest_hash[:24]}"
    instance_id = str(execution_instance_id or f"rbpg-exec-{uuid.uuid4().hex}").strip()
    if not instance_id:
        raise ValueError("execution_instance_id must be non-empty")
    return {
        "run_id": configuration_id,  # compatibility alias only
...
def make_replay_point_id(row_index: int, row: Mapping[str, Any]) -> str:
    source_id = row.get("source_record_id", row.get("utterance_id", row.get("Utterance_ID", row_index)))
    return f"{int(row_index)}:{source_id}"


def make_correlation_id(execution_instance_id: str, replay_point_id: str) -> str:
    return sha256_text(f"{execution_instance_id}|{replay_point_id}")[:32]
...
class DownstreamReceiptCollector:
    ...
    def emit(self, context: ReceiptContext) -> dict[str, Any]:
        if str(context.run_id) != str(context.configuration_id):
            raise ValueError("run_id compatibility alias must equal configuration_id")
        if not str(context.execution_instance_id).strip():
            raise ValueError("execution_instance_id must be non-empty")
        with self._lock:
            self._event_index += 1
            event_index = self._event_index
            attempt_key = (context.execution_instance_id, context.replay_point_id)
...
def reconcile_execution_receipts(trace_df: pd.DataFrame, receipt_df: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, Any]]:
    required_trace = {
        "run_id", "configuration_id", "execution_instance_id", "replay_point_id",
        "correlation_id", "authorized_to_generate", "generation_invoked",
    }
    ...
    if receipts.empty:
        receipts = pd.DataFrame(columns=[
            "run_id", "configuration_id", "execution_instance_id", "replay_point_id",
            "correlation_id", "receipt_digest",
        ])

    details: list[dict[str, Any]] = []
    trace_keys = set(zip(
        trace_df["configuration_id"].astype(str),
        trace_df["execution_instance_id"].astype(str),
        trace_df["replay_point_id"].astype(str),
    ))

    for trace_position, row in trace_df.reset_index(drop=True).iterrows():
        run_id = str(row["run_id"])
        configuration_id = str(row["configuration_id"])
        execution_instance_id = str(row["execution_instance_id"])
        replay_point_id = str(row["replay_point_id"])
        correlation_id = str(row["correlation_id"])
        point_receipts = receipts.loc[
            receipts["configuration_id"].astype(str).eq(configuration_id)
            & receipts["execution_instance_id"].astype(str).eq(execution_instance_id)
            & receipts["replay_point_id"].astype(str).eq(replay_point_id)
        ].copy()
        matching = point_receipts.loc[point_receipts["correlation_id"].astype(str).eq(correlation_id)]
        ...
        receipt_digests = sorted(point_receipts.get("receipt_digest", pd.Series(dtype=str)).astype(str).tolist())
        receipt_state_hashes = sorted(
            sha256_json({
                "replay_point_id": str(receipt.get("replay_point_id", "")),
                "downstream_operation": str(receipt.get("downstream_operation", "")),
                "attempt_index": int(receipt.get("attempt_index", 0)),
                "execution_status": str(receipt.get("execution_status", "")),
            })
        ...
        violation_count = sum([missing_receipt, unlogged_downstream_call, duplicate_downstream_call,
                               mismatched_correlation_id, unauthorized_downstream_call])
        details.append({
    ...
    if receipts.empty:
        orphan_receipts = receipts.copy()
    else:
        receipt_keys = list(zip(
            receipts["configuration_id"].astype(str), receipts["execution_instance_id"].astype(str),
            receipts["replay_point_id"].astype(str),
        ))
        orphan_receipts = receipts.loc[[key not in trace_keys for key in receipt_keys]]

    summary = {
        "trace_rows": int(len(trace_df)), "receipt_rows": int(len(receipts)),
        "orphan_receipts": int(len(orphan_receipts)),
        "missing_receipts": int(detail_df["missing_receipt"].sum()),
        "unlogged_downstream_calls": int(detail_df["unlogged_downstream_call"].sum()),
        "duplicate_downstream_calls": int(detail_df["duplicate_downstream_call"].sum()),
        "mismatched_correlation_ids": int(detail_df["mismatched_correlation_id"].sum()),
        "unauthorized_downstream_calls": int(detail_df["unauthorized_downstream_call"].sum()),
    }
    summary["receipt_validation_passed"] = int(
        summary["orphan_receipts"] == 0 and summary["missing_receipts"] == 0
        and summary["unlogged_downstream_calls"] == 0 and summary["duplicate_downstream_calls"] == 0
        and summary["mismatched_correlation_ids"] == 0 and summary["unauthorized_downstream_calls"] == 0
    )
    summary["authorization_execution_consistent"] = int(summary["receipt_validation_passed"] == 1)
    return detail_df, summary


def record_bound_trace_hash(trace_df: pd.DataFrame, reconciliation_df: pd.DataFrame) -> str:
    """Hash deterministic semantic execution state, excluding per-execution IDs."""
    ...
    for trace_position, row in trace_df.reset_index(drop=True).iterrows():
        ...
        canonical_records.append({
            ...
            "authorized_to_generate": int(row.get("authorized_to_generate", 0)),
            "primary_logged_execution": int(row.get("generation_invoked", 0)),
            "unauthorized_invocation": int(row.get("unauthorized_invocation", 0)),
            "receipt_count": int(receipt_values.get("receipt_count", 0)),
            "matching_receipt_count": int(receipt_values.get("matching_receipt_count", 0)),
            "receipt_state_hash_set": str(receipt_values.get("receipt_state_hash_set", "")),
            "receipt_consistent": int(receipt_values.get("receipt_consistent", 0)),
        })
    return sha256_json(canonical_records)


def config_bound_trace_hash(record_trace_hash: str, config_manifest_hash: str) -> str:
    return sha256_json({
        "schema_version": INTEGRITY_SCHEMA_VERSION,
...
def validate_receipt_digest_rows(receipt_df: pd.DataFrame) -> bool:
    """Recompute every semantic receipt digest and enforce the run-id alias."""
    if receipt_df.empty:
        return True
    required = {
        "schema_version", "run_id", "configuration_id", "execution_instance_id",
        "replay_point_id", "correlation_id", "downstream_operation", "attempt_index",
        "execution_status", "receipt_digest",
    }
    if required - set(receipt_df.columns):
        return False
    for row in receipt_df.to_dict(orient="records"):
        if str(row["run_id"]) != str(row["configuration_id"]):
            return False
        if not str(row["execution_instance_id"]).strip():
            return False
        semantic = {
