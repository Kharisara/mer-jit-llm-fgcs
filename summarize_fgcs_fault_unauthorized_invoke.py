#!/usr/bin/env python3
"""
Summarize the compact invocation-boundary fault-injection experiment.
...
against:
  unauthorized-invocation fault run:
    paper_outputs/fgcs_fault_unauthorized_invoke/determinism_hash_results.csv

It generates:
  paper_outputs/fgcs_tables_figures/fgcs_table_fault_unauthorized_invoke_per_run.csv
...
FAULT_DIR = Path("paper_outputs/fgcs_fault_unauthorized_invoke")
...
def main() -> None:
    ...
    merged = fault_sub[required_cols].merge(
        clean_sub[required_cols],
        on=keys,
        how="inner",
    ...
    clean_false_positive_runs = int((clean_sub["unauthorized_invocations"].astype(int) > 0).sum())
    ...
    summary = pd.DataFrame(
        [
            ...
            },
            {
                ...
                "detection_channel": "unauthorized-invocation counter + injected-fault counter",
                "interpretation": (
                    "Forced generator calls after action=0 are detected as invocation-boundary violations. "
                    "The policy action remains unchanged; the violation is measured at the invocation boundary."
                ),
            },
        ]
    )

    per_run_path = OUT_DIR / "fgcs_table_fault_unauthorized_invoke_per_run.csv"
    summary_path = OUT_DIR / "fgcs_table_fault_unauthorized_invoke_detection_summary.csv"

    per_run.to_csv(per_run_path, index=False)
    summary.to_csv(summary_path, index=False)

    print("[DONE] Invocation-boundary fault-injection summary generated.")
